...
if(isset($_POST['submit'])){
    ...
    if(empty($firstName)|| empty($lastName) || empty($email) || empty($password) || empty($repeatPassword)){
        header("Location: ../addNewMember.php?error=emptyfields&firstName=".$firstName."&lastName=".$lastName."&email".$email);
    exit();
   ...
   exit();
    ...
    //Create prepared statements for validation
        ...
        if(!mysqli_stmt_prepare($statement, $sql)){
            header("Location: ../addNewMember.php?error=sqlerror");
            exit();
        }else{
            ...
            if ($resultCheck > 0) {
                header("Location: ../addNewMember.php?error=usertaken&firstName=".$firstName."&lastName=".$lastName."&email".$email);
                exit();
            }else{
        ...
        if(!mysqli_stmt_prepare($statement, $sql)){
            ...
            mysqli_stmt_execute($statement);
            header("Location: ../addNewMember.php?error=success");
            exit();
            unset($_POST);
        }
            }
